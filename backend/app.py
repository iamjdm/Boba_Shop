...
#Adds flask route for submitting orders
...
def submit_order():
    try:
        ...
        db.session.execute(text("""
            INSERT INTO orders (customerID, orderDate, status, paymentMethod, totalAmount)
            VALUES (:customerID, :orderDate, :status, :paymentMethod, :totalAmount)
        """), {
...
#adds a route to save job applications into DB
...
def submit_job():
    try:
        ...
        query = text("""
            INSERT INTO jobapplications (positionID, name, email, experience)
            VALUES (:positionID, :name, :email, :experience)
        """)
...
#reads the job positions from database for dropdown menu
